...
for(my $i = 1; $i < scalar(@spreadsheet_rows); $i ++)
{
	...
	$english_text_raw =~ s/\[SPACE\]/ \[SPACE\] /g;
	...
	else
	{
		...
		for(my $j = 0; $j < scalar(@english_text_words); $j ++)
		{
			...
			# Process forced whitespace.
			elsif($english_text_words[$j] eq "[SPACE]")
			{
				$english_text_hex .= "20";
			}
	...
	# Use custom tile spacing for "a.k.a.".
	if($file_input_basename eq "S_G_03.HBN_002_ATL")
	{
		$english_text_hex =~ s/612E6B2E61/61FF23042EFF23006BFF23042EFF230061/gi;
	}
